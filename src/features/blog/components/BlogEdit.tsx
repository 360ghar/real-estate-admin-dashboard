...
const BlogPostMetaForm: React.FC<{ form: ReturnType<typeof useForm<BlogPostForm>>; images: string[]; setImages: (urls: string[]) => void; categoryOptions: { value: string; label: string }[]; tagOptions: { value: string; label: string }[] }> = ({ form, images, setImages, categoryOptions, tagOptions }) => (
  <>
    ...
    <div className="md:col-span-2"><FormField control={form.control} name="active" render={({ field }) => (<FormItem className="flex items-center justify-between rounded-md border p-4"><div className="space-y-1"><FormLabel>Publish status</FormLabel><p className="text-sm text-muted-foreground">Turn this on to make the post visible on the site. Turn it off to keep it as a draft.</p></div><FormControl><Switch checked={field.value ?? false} onCheckedChange={field.onChange} /></FormControl></FormItem>)} /></div>
  </>
)
...
const BlogEdit: React.FC<BlogEditProps> = ({ identifier, onSuccess }) => {
  ...
  const form = useForm<BlogPostForm>({ resolver: zodResolver(blogPostSchema), defaultValues: { title: '', content: '', excerpt: '', cover_image_url: '', categories: [], tags: [], active: false } })
  const { setValue } = form
  ...
  useEffect(() => {
    if (post) {
      ...
      setValue('tags', post.tags?.map((tag) => tag.slug) || []); setValue('active', !!post.active)
