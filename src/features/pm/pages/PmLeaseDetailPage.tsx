...
import { AlertCircle, FileUp, RefreshCw, ShieldX } from "lucide-react";
import {
  type DocumentType,
  type LeaseRenew,
  useGetPmLeaseQuery,
...
export default function PmLeaseDetailPage() {
  ...
  const submitSignedUpload = async () => {
    ...
    if (!signedFile) {
      toast({
        title: "Missing file",
        description: "Choose a signed lease PDF.",
        variant: "destructive",
      });
      return;
    }
  ...
  const submitRenew = async () => {
    ...
    try {
      ...
      toast({
        title: "Failed",
        description: getErrorMessage(e, "Could not renew lease."),
        variant: "destructive",
      });
    }
  };
  ...
  const handleTerminateConfirm = useCallback(async () => {
    if (!lease.data) return;
    try {
      await terminateLease(lease.data.id).unwrap();
      toast({ title: "Terminated", description: "Lease terminated." });
    } catch (e: unknown) {
      toast({
        title: "Failed",
        description: getErrorMessage(e, "Could not terminate lease."),
        variant: "destructive",
      });
    } finally {
      setTerminateOpen(false);
    }
  }, [lease.data, terminateLease, toast]);

  // Reset upload form state when dialog closes
  ...
  // Reset renew form state when dialog closes
  const handleRenewOpenChange = useCallback((open: boolean) => {
    setRenewOpen(open);
    if (!open) {
      setRenewStart("");
      setRenewEnd("");
  ...
  if (!leaseIdNum || Number.isNaN(leaseIdNum)) {
    return (
      <div className="text-sm text-muted-foreground">Invalid lease id.</div>
    );
  }
  ...
  if (lease.isError) {
    return (
      <div className="flex items-center gap-2 rounded-md border border-destructive/50 bg-destructive/10 p-4 text-sm text-destructive">
        ...
        <span>Failed to load lease.</span>
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          {lease.data ? (
            <Badge variant="secondary">{lease.data.status}</Badge>
          ) : null}
              ...
              <div className="space-y-4">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <Select
                      value={signedByOwner}
                      onValueChange={(v) => setSignedByOwner(v as "yes" | "no")}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      value={signedByTenant}
                      onValueChange={(v) =>
                        setSignedByTenant(v as "yes" | "no")
                      }
                    >
                      <SelectTrigger>
                        <SelectValue />
                ...
                <div className="flex justify-end gap-2">
                  ...
                  <Button
                    onClick={() => {
                      void submitSignedUpload();
                    }}
                    disabled={
                      uploadDocState.isLoading ||
                      uploadSignedState.isLoading ||
                      !signedFile
                    }
                  >
                    {uploadDocState.isLoading || uploadSignedState.isLoading
                      ? "Uploading…"
                      : "Upload"}
                  </Button>
                </div>
          ...
          <Dialog open={renewOpen} onOpenChange={handleRenewOpenChange}>
            <DialogTrigger asChild>
              <Button variant="outline">
            ...
            <DialogContent className="max-w-xl">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div className="space-y-2 md:col-span-2">
                  ...
                  <Select
                    value={makeActive}
                    onValueChange={(v) => setMakeActive(v as "yes" | "no")}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
            ...
            {lease.isLoading ? (
              ...
              <>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-medium">
                    {lease.data.payment_due_day}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-muted-foreground">Grace period</span>
                  <span className="font-medium">
                    {lease.data.grace_period_days} days
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  ...
                  {lease.data.lease_document_id ? (
                    <span className="font-medium">
                      Document #{lease.data.lease_document_id}
                    </span>
                  ) : (
          ...
          <CardContent className="space-y-2">
            {lease.data ? (
              <>
                ...
                >
                  <Link to={`/pm/properties/${lease.data.property_id}`}>
                    Open property
                  </Link>
                </Button>
                <Button
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              end the lease agreement for{" "}
              {lease.data?.tenant_name || "the current tenant"} and cannot be
              undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
