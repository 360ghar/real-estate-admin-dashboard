...
export default function PmPropertiesPage() {
  ...
  const [propertyType, setPropertyType] = useState<'apartment' | 'house' | 'builder_floor' | 'room'>('apartment')
  ...
  const [lateFeePolicyJson, setLateFeePolicyJson] = useState('{"type":"fixed","amount":0}')

  const submitCreate = async () => {
    if (!ownerId) {
      toast({ title: 'Select an owner', description: 'Choose an owner from the top bar.', variant: 'destructive' })
      return
    }
    if (!title || !basePrice) {
      toast({ title: 'Missing fields', description: 'Title and base price are required.', variant: 'destructive' })
      return
    }
    let lateFeePolicy: Record<string, unknown> | undefined
    try {
      lateFeePolicy = lateFeePolicyJson ? (JSON.parse(lateFeePolicyJson) as Record<string, unknown>) : undefined
    } catch {
    ...
    try {
      ...
      await updateProperty({ property_id: created.id, payload: { late_fee_policy: lateFeePolicy } }).unwrap()
      ...
      toast({ title: 'Failed', description: getErrorMessage(e, 'Could not create property.'), variant: 'destructive' })
    }
  }
  ...
  const columns = useMemo<ColumnDef<PmProperty>[]>(() => {
    return [
        ...
        cell: ({ row }) => (
          <div className="min-w-0">
            ...
            <div className="truncate text-xs text-muted-foreground">
              {(row.original.locality || row.original.city || row.original.full_address || '').toString() || '—'}
            </div>
          </div>
        ),
      },
      {
        ...
        header: 'Occupancy',
        cell: ({ row }) => (
          <Badge variant={row.original.current_lease_id ? 'default' : 'outline'}>
      ...
      },
      {
        ...
        header: 'Due Day',
        cell: ({ row }) => <span className="text-sm">{row.original.payment_due_day ?? '—'}</span>,
      },
      {
        id: 'actions',
        header: '',
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-sm text-muted-foreground">
              {role === 'admin' ? 'All managed properties.' : 'Managed properties for the selected owner.'}
            </p>
          </div>
              ...
              <DialogContent className="max-w-3xl">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <div className="space-y-2 md:col-span-2">
                    <Label>Title</Label>
                    <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. 2BHK in Indiranagar" />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input value={basePrice} onChange={(e) => setBasePrice(e.target.value)} placeholder="e.g. 25000" />
                  </div>
                  <div className="space-y-2">
                ...
                {role === 'admin' && !ownerId ? (
                  <div className="text-sm text-muted-foreground">
                    Select an owner from the top bar to create a managed property in the correct portfolio.
                  </div>
                ) : null}
              </DialogContent>
            </Dialog>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid gap-3 md:grid-cols-3">
              <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search title/address…" />
            ...
            {properties.isLoading ? (
              ...
              <EmptyState title="No managed properties" description="Try adjusting filters." />
            )}
          </CardContent>
        </Card>
      </div>
