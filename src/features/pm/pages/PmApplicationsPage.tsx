...
import { Copy, FileSearch, Plus } from 'lucide-react'
...
const buildPublicFormUrl = (slug: string) => {
  const apiBase = (import.meta.env.VITE_API_BASE_URL as string | undefined) ?? 'http://localhost:8000/api/v1'
...
export default function PmApplicationsPage() {
  ...
  const forms = useListApplicationFormsQuery(
    { owner_id: ownerId, q: debouncedFormsQ || undefined, limit: formsLimit, offset: formsOffset },
  ...
  const applications = useListApplicationsQuery(
    { owner_id: ownerId, status: status || undefined, limit: appsLimit, offset: appsOffset },
    { skip: role === 'agent' && !ownerId },
  )
    ...
    return [
      {
        ...
        cell: ({ row }) => (
          <div className="min-w-0">
            ...
            <div className="truncate text-xs text-muted-foreground">Slug: {row.original.slug}</div>
          </div>
        ),
      },
      {
        ...
        header: 'Property',
        cell: ({ row }) => <span className="text-sm">{row.original.property_id ? `#${row.original.property_id}` : 'Any'}</span>,
      },
      {
        accessorKey: 'is_active',
        header: 'Active',
        cell: ({ row }) => <Badge variant={row.original.is_active ? 'default' : 'outline'}>{row.original.is_active ? 'yes' : 'no'}</Badge>,
      },
      {
        id: 'actions',
        header: '',
        cell: ({ row }) => (
          <div className="flex justify-end gap-2">
            <Button asChild variant="outline" size="sm">
              <Link to={`/pm/applications?tab=inbox&form_id=${row.original.id}`}>Inbox</Link>
            </Button>
            <Button
              variant="outline"
              size="sm"
        ...
        cell: ({ row }) => (
          <div className="min-w-0">
            ...
            <div className="truncate text-xs text-muted-foreground">
              Form #{row.original.form_id} • Property #{row.original.property_id}
            </div>
          </div>
        ),
      },
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="min-w-0">
            ...
            <div className="truncate text-xs text-muted-foreground">{row.original.applicant_phone || row.original.applicant_email || ''}</div>
          </div>
        ),
      },
      {
        ...
        header: 'Submitted',
        cell: ({ row }) => (row.original.submitted_at ? new Date(row.original.submitted_at).toLocaleString() : '—'),
      },
      {
        id: 'actions',
        header: '',
        cell: ({ row }) => (
          <div className="flex justify-end gap-2">
            ...
            <Button
              size="sm"
              disabled={decideState.isLoading || row.original.status === 'approved'}
              onClick={() => {
                void (async () => {
                  try {
                    await decideApplication({ application_id: row.original.id, payload: { decision: 'approved' } }).unwrap()
            ...
            <Button
              size="sm"
              variant="destructive"
              disabled={decideState.isLoading || row.original.status === 'rejected'}
              onClick={() => {
                void (async () => {
                  try {
                    await decideApplication({ application_id: row.original.id, payload: { decision: 'rejected' } }).unwrap()
  ...
  }, [decideApplication, decideState.isLoading, toast])
  ...
  const submitCreateForm = async () => {
    ...
    try {
      questions = questionsJson ? (JSON.parse(questionsJson) as Record<string, unknown>) : undefined
    } catch {
    ...
    if (!effectiveOwnerId) {
      toast({ title: 'Select an owner', description: 'Choose an owner from the top bar or pick a property.', variant: 'destructive' })
      return
    }
    ...
    const payload: RentalApplicationFormCreate = {
      ...
      application_fee_amount: applicationFeeAmount ? Number(applicationFeeAmount) : undefined,
      questions,
    }
    ...
    try {
      ...
      toast({ title: 'Failed', description: getErrorMessage(e, 'Could not create form.'), variant: 'destructive' })
    }
  }
      ...
      <div className="space-y-6">
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-sm text-muted-foreground">Create forms, share links, and process submissions.</p>
          </div>
            ...
            <DialogContent className="max-w-3xl">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  <Input value={applicationFeeAmount} onChange={(e) => setApplicationFeeAmount(e.target.value)} placeholder="e.g. 500" />
                </div>
                <div className="space-y-2 md:col-span-2">
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <Button onClick={() => { void submitCreateForm() }} disabled={createState.isLoading}>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Button variant={tab === 'forms' ? 'default' : 'outline'} size="sm" onClick={() => setTab('forms')}>
            Forms
          </Button>
        ...
        {tab === 'forms' ? (
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="grid gap-3 md:grid-cols-3">
                <Input value={formsQ} onChange={(e) => { setFormsQ(e.target.value); setFormsOffset(0) }} placeholder="Search title…" />
                <Select value={String(formsLimit)} onValueChange={(v) => { setFormsLimit(Number(v)); setFormsOffset(0) }}>
                  <SelectTrigger>
                ...
                <>
                  ...
                  <div className="flex items-center justify-between pt-2">
                    ...
                    <div className="flex gap-2">
                      <Button variant="outline" size="sm" disabled={!formsCanPrev} onClick={() => setFormsOffset(Math.max(0, formsOffset - formsLimit))}>
                        Prev
                      </Button>
                      <Button variant="outline" size="sm" disabled={!formsCanNext} onClick={() => setFormsOffset(formsOffset + formsLimit)}>
                        Next
                      </Button>
                    </div>
                  </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="grid gap-3 md:grid-cols-3">
                <Select value={status} onValueChange={(v) => { setStatus(v as TenantStatus | ''); setAppsOffset(0) }}>
                  <SelectTrigger>
                ...
                <Select value={String(appsLimit)} onValueChange={(v) => { setAppsLimit(Number(v)); setAppsOffset(0) }}>
                  <SelectTrigger>
                    <SelectValue placeholder="Page size" />
                  </SelectTrigger>
                  <SelectContent>
              ...
              {applications.isLoading ? (
                ...
                <>
                  ...
                  <div className="flex items-center justify-between pt-2">
                    ...
                    <div className="flex gap-2">
                      <Button variant="outline" size="sm" disabled={!appsCanPrev} onClick={() => setAppsOffset(Math.max(0, appsOffset - appsLimit))}>
                        Prev
                      </Button>
                      <Button variant="outline" size="sm" disabled={!appsCanNext} onClick={() => setAppsOffset(appsOffset + appsLimit)}>
                        Next
                      </Button>
                    </div>
                  </div>
